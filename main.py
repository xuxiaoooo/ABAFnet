...
hdf = pd.read_csv('filtered_validationClass.csv').reset_index(drop=True)[['class', 'cust_id']]
class_0 = hdf[hdf['class'] == 0]
class_1 = hdf[hdf['class'] == 1]
random_state = 2 #njmu seed 3855
class_0_resampled = class_0.sample(len(class_1), random_state=random_state)
...
features = pd.read_csv('validate_emo_large.csv').drop(columns=['class'])
...
for i in range(len(hdf)):
    data_list_img1.append('/home/user/xuxiao/DeepL/validation-image-features/' + str(data['cust_id'][i]) + '/envelope.png')
    data_list_img2.append('/home/user/xuxiao/DeepL/validation-image-features/' + str(data['cust_id'][i]) + '/spectrogram.png')
    data_list_img3.append('/home/user/xuxiao/DeepL/validation-image-features/' + str(data['cust_id'][i]) + '/mel_spectrogram.png')
...
for params in ParameterGrid(param_grid):
    ...
    plt.tight_layout()
    plt.savefig('ROC_curvex_njmu.png', dpi=300)
